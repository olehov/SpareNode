...
enum class HttpResponseValidationErrorCode : std::uint8_t
{
    ...
    managed_date_header        ///< Date is generated by the response transport.
};
...
class HttpResponse final
{
  public:
    ...
    /// @brief Returns the immutable generated Date value for final responses.
    /// @return IMF-fixdate, or empty for an informational response.
    [[nodiscard]] std::string_view date_value() const noexcept
    {
        return date_value_;
    }
  ...
  private:
    ...
    std::string date_value_;     ///< UTC origination date retained across serialization calls.
